...
namespace ModularFirearms {
    ...
    public class FirearmFunctions {
        ...
        public static EffectData bloodHitData = Catalog.GetData<EffectData>("HitRagdollOnFlesh", true);
        public static EffectData data = Catalog.GetData<EffectData>("HitBladeDecalFlesh", true);
        ...
        // New Stuff
        public static IEnumerator ShootCoroutine(Item item, FirearmModule module, BaseFirearmGenerator gun, TrackFiredDelegate TrackedFire, IsFiringDelegate WeaponIsFiring, FireMode fireSelector) {
            WeaponIsFiring?.Invoke(true);
            float fireDelay = 60.0f / (float)module.fireRate;
        ...
        public static void DoRayCast(Item item, Transform raycastPoint, float range, float damage, float force) {
            ...
            if (raycastHit.rigidbody != null) {
                RagdollPart ragdollPart = raycastHit.rigidbody.GetComponentInParent<RagdollPart>();
                if (ragdollPart == null) {
                    ...
                    if (ragdollPart.ragdoll.creature.state != Creature.State.Dead) {
                        CollisionInstance coll = new CollisionInstance(new DamageStruct(DamageType.Pierce, damage), null, null);
                        coll.contactPoint = raycastHit.point;
                        coll.contactNormal = raycastHit.normal;
                        ragdollPart.ragdoll.creature.Damage(coll);
                    }
                    ragdollPart.rb.AddForce(force * ray.direction, ForceMode.Impulse);
                }
            }
        }
        ...
        private static void SpawnBulletHole(RaycastHit raycastHit, RagdollPart ragdollPart) {
            EffectModuleReveal effectModuleReveal = data.modules[3] as EffectModuleReveal;
            List<RevealMaterialController> revealMaterialControllers = new List<RevealMaterialController>();
            foreach (Creature.RendererData renderer in ragdollPart.renderers) {
                if (renderer.revealDecal && ((renderer.revealDecal.type == RevealDecal.Type.Default && effectModuleReveal.typeFilter.HasFlag(EffectModuleReveal.TypeFilter.Default)) || (renderer.revealDecal.type == RevealDecal.Type.Body && effectModuleReveal.typeFilter.HasFlag(EffectModuleReveal.TypeFilter.Body)) || (renderer.revealDecal.type == RevealDecal.Type.Outfit && effectModuleReveal.typeFilter.HasFlag(EffectModuleReveal.TypeFilter.Outfit)))) {
            ...
            GameObject reveal = new GameObject();
            reveal.transform.position = raycastHit.point;
            reveal.transform.rotation = Quaternion.LookRotation(raycastHit.normal);
            EffectInstance bulletHitEffect = bloodHitData.Spawn(reveal.transform.position, reveal.transform.rotation, null, null, true, new Type[0]);
            bulletHitEffect.SetIntensity(10f);
            bulletHitEffect.Play(0);
            Vector3 direction = -reveal.transform.forward;
            GameManager.local.StartCoroutine(RevealMaskProjection.ProjectAsync(reveal.transform.position + -direction * effectModuleReveal.offsetDistance, direction, reveal.transform.up, effectModuleReveal.depth, effectModuleReveal.maxSize, effectModuleReveal.maskTexture, effectModuleReveal.maxChannelMultiplier, revealMaterialControllers, effectModuleReveal.revealData, null));
            TriggerRealisticBleed(ragdollPart, raycastHit.collider, reveal.transform);
        }

        // New
        public static void TriggerRealisticBleed(RagdollPart ragdollPart, Collider collider, Transform location) {
            ...
            effectInstance.AddEffect(data, location.position, location.rotation, location, collisionInstance, true, new Type[0]);
        }
        ...
        public static IEnumerator TransferDeltaEnergy(Imbue itemImbue, SpellCastCharge activeSpell, float energyDelta = 20.0f, int counts = 5) {
            if (activeSpell != null) {
                for (int i = 0; i < counts; i++) {
                    try { itemImbue.Transfer(activeSpell, energyDelta); } catch { }
